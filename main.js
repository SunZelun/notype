...
const BASE_WINDOWS_APP_ID = "com.notype.app";
...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `NOTYPE-${APP_CHANNEL}`);
...
async function startApp() {
  ...
  // Create the floating dictation window first. The settings window opens on demand.
  ...
  trayManager.setWindows(windowManager.mainWindow, windowManager.settingsWindow);
  trayManager.setWindowManager(windowManager);
  trayManager.setCreateSettingsCallback(async () => {
    await windowManager.openSettingsWindow();
    trayManager.setWindows(windowManager.mainWindow, windowManager.settingsWindow);
    updateManager.setWindows(windowManager.mainWindow, windowManager.settingsWindow);
  });
  ...
  if (process.platform === "darwin") {
    ...
    globeKeyManager.on("globe-down", async () => {
      ...
      // Forward to settings window for hotkey capture
      if (isLiveWindow(windowManager.settingsWindow)) {
        windowManager.settingsWindow.webContents.send("globe-key-pressed");
      }
    ...
    globeKeyManager.on("globe-up", async () => {
      ...
      // Forward to settings window for hotkey capture (Fn key released)
      if (isLiveWindow(windowManager.settingsWindow)) {
        windowManager.settingsWindow.webContents.send("globe-key-released");
      }
...
if (gotSingleInstanceLock) {
  app.on("second-instance", async (_event, commandLine) => {
    ...
    if (isLiveWindow(windowManager.settingsWindow)) {
      if (windowManager.settingsWindow.isMinimized()) {
        windowManager.settingsWindow.restore();
      }
      windowManager.settingsWindow.show();
      windowManager.settingsWindow.focus();
      if (windowManager.settingsWindow.webContents.isCrashed()) {
        windowManager.loadSettingsWindow();
      }
    } else {
  ...
  app.on("browser-window-focus", (event, window) => {
    // Only apply always-on-top to the dictation window, not the settings window
    ...
    // Settings behaves like a normal window.
  });
  ...
  app.on("activate", () => {
    ...
    if (BrowserWindow.getAllWindows().length === 0) {
      ...
      // Show settings when the dock icon is clicked.
      if (windowManager && isLiveWindow(windowManager.settingsWindow)) {
        // Ensure dock icon is visible when settings opens
        if (process.platform === "darwin" && app.dock) {
          app.dock.show();
        }
        if (windowManager.settingsWindow.isMinimized()) {
